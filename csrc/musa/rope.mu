...
torch::Tensor rope_apply_sm90(
    ...
    torch::Tensor sin,
    double sin_sign) {
  TORCH_CHECK(x.device().type() == c10::kPrivateUse1, "rope: x must be a CUDA tensor");
  TORCH_CHECK(x.dim() == 2, "rope: x must be 2-D [n_rows, D]");
  TORCH_CHECK(x.is_contiguous(), "rope: x must be contiguous");
  TORCH_CHECK(cos.device().type() == c10::kPrivateUse1 && sin.device().type() == c10::kPrivateUse1, "rope: cos/sin must be CUDA tensors");

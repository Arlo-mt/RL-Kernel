...
void check_deterministic_attention_inputs(
  ...
  TORCH_CHECK(q.device().type() == c10::kPrivateUse1 && k.device().type() == c10::kPrivateUse1 && v.device().type() == c10::kPrivateUse1,
  ...
  if (key_padding_mask.has_value() && key_padding_mask->defined()) {
    ...
    TORCH_CHECK(mask.device().type() == c10::kPrivateUse1, "deterministic_attention: key_padding_mask must be CUDA");
...
std::vector<torch::Tensor> deterministic_attention_forward_impl(
  ...
  const at::musa::OptionalMUSAGuard device_guard(at::device_of(q));

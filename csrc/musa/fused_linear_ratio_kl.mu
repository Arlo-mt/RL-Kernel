...
__global__ void merge_policy_tile_kernel(
    const float* logits,
...
__global__ void linear_ratio_kl_epilogue_kernel(
    ...
    const int32_t* mask,
    const float* old_logp,
    const float* reference_logp,
    float* ratio,
    float* kl,
    float* diff,
    float* policy_lse,
...
__global__ void linear_ratio_kl_dlogits_kernel(
    const float* logits,
    const float* policy_lse,
    const float* ratio,
    const float* diff,
    const float* grad_ratio,
    const float* grad_kl,
    const int64_t* target,
    const int32_t* mask,
    float* dlogits,
    ...
    float gradient = -coefficient * probability;
    if (target[row] == vocab_start + column) {
...
std::vector<torch::Tensor> fused_linear_ratio_kl_musa_forward(
    ...
    for (int64_t vocab_start = 0; vocab_start < vocab; vocab_start += kVocabTile) {
        ...
        merge_policy_tile_kernel<<<rows, kBlockSize, 0, stream>>>(
    ...
    auto stream = at::musa::getCurrentMUSAStream();
    linear_ratio_kl_epilogue_kernel<<<
        (rows + kBlockSize - 1) / kBlockSize,
        kBlockSize,
        0,
        stream>>>(
        row_max.data_ptr<float>(),
        ...
        reference_logp.data_ptr<float>(),
        ratio.data_ptr<float>(),
        kl.data_ptr<float>(),
        diff.data_ptr<float>(),
        policy_lse.data_ptr<float>(),
        rows);
    C10_MUSA_KERNEL_LAUNCH_CHECK();
...
std::vector<torch::Tensor> fused_linear_ratio_kl_musa_backward(
    ...
    for (int64_t vocab_start = 0; vocab_start < vocab; vocab_start += kVocabTile) {
        ...
        auto logits = project_tile(hidden_f, weight, bias, vocab_start, vocab_count);
        auto dlogits =
            torch::empty({rows, vocab_count}, hidden.options().dtype(torch::kFloat));
        const int total = rows * vocab_count;
        auto stream = at::musa::getCurrentMUSAStream();
        linear_ratio_kl_dlogits_kernel<<<
            ...
            ratio.data_ptr<float>(),
            diff.data_ptr<float>(),
            grad_ratio.data_ptr<float>(),
            grad_kl.data_ptr<float>(),
            target.data_ptr<int64_t>(),
            mask.data_ptr<int32_t>(),

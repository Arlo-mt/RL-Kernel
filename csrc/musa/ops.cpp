...
void check_ratio_forward_inputs(const torch::Tensor &policy,
...
std::vector<torch::Tensor> ratio_kl_musa_forward(torch::Tensor policy,
                                                 ...
                                                 torch::Tensor mask,
                                                 torch::Tensor old_logp) {
  check_ratio_forward_inputs(policy, reference, action, mask, old_logp);
...
torch::Tensor grpo_group_advantages_musa(torch::Tensor rewards,
                                         torch::Tensor bounds,
                                         double eps) {
  TORCH_CHECK(rewards.device().type() == c10::kPrivateUse1,
              "rewards must be a MUSA tensor");
  TORCH_CHECK(bounds.device() == rewards.device(),
              "bounds must be on the same MUSA device as rewards");
  TORCH_CHECK(rewards.dim() == 1, "rewards must be 1D");
  TORCH_CHECK(bounds.dim() == 1 && bounds.numel() >= 2,
              "bounds must be 1D with length num_groups + 1");
  TORCH_CHECK(rewards.scalar_type() == at::ScalarType::Float,
              "rewards must be FP32");
  TORCH_CHECK(bounds.scalar_type() == at::ScalarType::Int,
              "bounds must be int32");
  return grpo_group_advantages_musa_impl(rewards.contiguous(),
                                         bounds.contiguous(), eps);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("ratio_kl_musa_forward", &ratio_kl_musa_forward);
  m.def("ratio_kl_musa_backward", &ratio_kl_musa_backward);
  m.def("grpo_group_advantages_musa", &grpo_group_advantages_musa);
}

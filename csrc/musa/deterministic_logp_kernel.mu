...
void check_deterministic_logp_inputs(
    ...
    TORCH_CHECK(logits.device().type() == c10::kPrivateUse1, "logits must be a CUDA tensor");
    TORCH_CHECK(token_ids.device().type() == c10::kPrivateUse1, "token_ids must be a CUDA tensor");
    TORCH_CHECK(output.device().type() == c10::kPrivateUse1, "output must be a CUDA tensor");
    TORCH_CHECK(
...
void check_deterministic_logp_indices(
    ...
    TORCH_CHECK(row_indices.device().type() == c10::kPrivateUse1, "row_indices must be a CUDA tensor");
    TORCH_CHECK(

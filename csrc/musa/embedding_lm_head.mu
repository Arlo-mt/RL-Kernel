...
void check_sm90_device() {
    // The port uses generic warp/reduction instructions, not NVIDIA SM90 PTX.
}

void check_cuda_same_device(torch::Tensor a, torch::Tensor b, const char *a_name,
                            const char *b_name) {
    TORCH_CHECK(a.device().type() == c10::kPrivateUse1 && b.device().type() == c10::kPrivateUse1, a_name, " and ", b_name,
...
torch::Tensor embedding_sm90_forward_impl(torch::Tensor token_ids, torch::Tensor weight,
                                          bool output_fp32) {
    TORCH_CHECK(token_ids.device().type() == c10::kPrivateUse1 && weight.device().type() == c10::kPrivateUse1,
        ...
        at::kHalf, at::kBFloat16, weight.scalar_type(), "embedding_sm90_forward", [&] {
            if (output_fp32) {
                embedding_sm90_forward_kernel<scalar_t, float>
                    <<<blocks, kThreads, 0, at::musa::getCurrentMUSAStream()>>>(
...
torch::Tensor lm_head_sm90_forward_impl(torch::Tensor hidden, torch::Tensor weight,
    ...
    c10::musa::MUSAGuard device_guard(hidden.device());
    check_sm90_device();
    ...
    if (bias.has_value()) {
        TORCH_CHECK(bias->device().type() == c10::kPrivateUse1, "lm_head bias must be a CUDA tensor");
        ...
        at::kHalf, at::kBFloat16, compute_dtype, "lm_head_sm90_forward", [&] {
            if (output_fp32) {
                lm_head_sm90_forward_kernel<scalar_t, float>
                    <<<static_cast<int>(total), kThreads, 0, at::musa::getCurrentMUSAStream()>>>(

...
def test_resolve_logprob_threshold_rejects_dtype_outside_ws1_contract(monkeypatch):
    ...
    for invalid in (True, -1.0, float("nan"), float("inf"), "0.1"):
        contract = load_contract()
        contract["accuracy"]["default"]["logprob"]["float32"]["atol"] = invalid
        monkeypatch.setattr(tolerance_module, "load_contract", lambda value=contract: value)
        with pytest.raises(ValueError, match="invalid WS1 logprob threshold"):
            resolve_logprob_threshold("float32")

...
class _MusaRatioKLFunction(torch.autograd.Function):
    ...
    def forward(ctx, policy_logits, ref_logits, action_ids, attention_mask, old_logps):
        ...
        mask = attention_mask.contiguous().view(-1).to(device=policy.device, dtype=torch.int32)
        action = action_ids.contiguous().view(-1).to(device=policy.device, dtype=torch.long)
        ...
        old = old_logps.contiguous().view(-1).to(device=policy.device, dtype=torch.float32)
...
class MusaRatioKLOp:
    ...
    def __init__(self) -> None:
        ...
        if missing:
            raise RuntimeError(f"MUSA ratio_kl extension is missing: {', '.join(missing)}")
